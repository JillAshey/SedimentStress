...
all(rownames(metadata_acerv) %in% colnames(acerv_counts_filt)) # must come out TRUE
# Set Treatment as a factor
metadata_acerv$Treatment <- factor(metadata_acerv$Treatment, levels = c("control", "Treatment1", "Treatment2", "Treatment3", "Treatment4"))
# create matrix that can be read in DESeq
data <- DESeqDataSetFromMatrix(countData = acerv_counts_filt, colData = metadata_acerv, design = ~ Treatment)
...
DEG_T1_vs_T4.vst.sig <- varianceStabilizingTransformation(DEG_T1_vs_T4.sig.list, blind = FALSE) # apply a regularized logtransformation to minimize effects of small counts and normalize wrt library. Specify fit type??
## error from varianceStabilizingTransformation:
...
## Not sure what this error means?????? it worked before
### FIGURE THIS OUT --JA 20211221
