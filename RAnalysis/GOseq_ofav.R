...
gcounts_filt_ofav <- read.csv("~/Desktop/PutnamLab/Repositories/SedimentStress/SedimentStress/Output/DESeq2/ofav_counts_filt.csv", header = TRUE)
...
DEG_ofav <- read.csv("~/Desktop/PutnamLab/Repositories/SedimentStress/SedimentStress/Output/DESeq2/ofav_unique.sig.list.csv", header = TRUE)
dim(DEG_ofav)
...
map$gene_id <- gsub(";.*", "", map$gene_id) # isolated LOC term
...
ref <- subset(ref, id == "CDS") # subsetting by CDS because it has the XP protein designation. GO annot terms also have that designation, so I can merge those files 
ref$prot <- sub(";.*", "", ref$attr)
ref$prot <- gsub(".*-", "", ref$prot)
ref$gene_id <- regmatches(ref$attr, gregexpr("(?<=gene=).*", ref$attr, perl = TRUE))
ref$gene_id <- gsub(";.*", "", ref$gene_id) # removing everything after LOC term
...
dim(ofav_filt.map_unique) # should be same # of rows as gcounts_filt_acerv

#Find gene positions in ref corresponding to expressed genes 
ofav_filt.map_unique.ref <- merge(ofav_filt.map_unique, ref, by = "gene_id", all.x= TRUE)
...
# may have to do unique() at some point above
