...
ggsave("~/Desktop/acerv_sub_DEGs_PCA.pdf", acerv_sub_DEG_PCA_plot)
...
#unique.DEG.annot <- unique.DEG.annot[,2:14]
all(rownames(acerv_metadata_sub) %in% colnames(unique.DEG.annot)) # must come out TRUE
...
all(rownames(df) %in% colnames(mat)) # must come out TRUE

col_clust <- test$Treatment
