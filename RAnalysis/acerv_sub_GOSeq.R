# Title: Sediment stress - acerv sub GOseq
# Author: Jill Ashey
# date: 12/1/20
...
colnames(acerv_counts_filt)[1] <-"gene_id"
...
acerv_filt.map_unique <- merge(acerv_counts_filt, map_unique, by = "gene_id", all.x = TRUE)
...
#Make gene vector
DEG <- filter(acerv_genes.map_unique.ref, gene_id%in%DEG_acerv_sub$gene_id) #make vector of differentially expressed genes
dim(DEG) #should be 217
...
write.csv(GO.terms, file = "~/Desktop/acerv_sub_GOterms.unique.csv")
...
write.csv(GO.wall, file = "~/Desktop/acerv_sub_GO_ALL.csv")
...
write.csv(MF, file = "~/Desktop/acerv_sub_MF_Sig_Enriched_GO.05.csv")
write.csv(CC, file = "~/Desktop/acerv_sub_CC_Sig_Enriched_GO.05.csv")
write.csv(BP, file = "~/Desktop/acerv_sub_BP_Sig_Enriched_GO.05.csv")
write.csv(enriched.GO.05, file = "~/Desktop/acerv_sub_Sig_Enriched_GO.05_ALL.csv")
...
#GO.terms_gene_ids.acerv_genes.map_unique.ref <- na.omit(GO.terms_gene_ids.acerv_genes.map_unique.ref)
write.csv(GO.terms_gene_ids.acerv_genes.map_unique.ref, file = "~/Desktop/acerv_sub_GOterms_gene_ids.csv")
...
ggsave("~/Desktop/acerv_sub_GOplot_05.pdf", GOplot, width = 21, height = 21, units = c("in"))
...
GOplot2
ggsave("~/Desktop/acerv_sub_GOplot2_05.pdf", GOplot2, width = 28, height = 28, units = c("in"))
...
GOplot2_pvalue <- enriched.GO.05 %>% drop_na(ontology) %>% mutate(term = fct_reorder(term, over_represented_pvalue)) %>%
  ...
  theme(panel.border = element_blank(), # Set border
        ...
        plot.background=element_blank()) #Set the plot background #set title attributes
GOplot2_pvalue
ggsave("~/Desktop/acerv_sub_GOplot2_pvalue_05.pdf", GOplot2_pvalue, width = 28, height = 28, units = c("in"))
