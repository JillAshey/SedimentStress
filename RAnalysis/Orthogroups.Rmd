...
ortho <- na.omit(ortho)
colnames(ortho) <- c("Orthogroup", "Acervicornis", "Ofaveolata", "Mcavernosa", "Pacuta", "Plobata")
```
...
length(unique(acerv$gene_id)) # 60 DEGs found in orthogroups 
length(unique(acerv$Orthogroup)) # 60 orthogroups ? should they be the same?
```
...
length(unique(mcav$gene_id)) # 29 DEGs found in orthogroups 
length(unique(mcav$Orthogroup)) # 26 orthogroups 
```
...
venn.plot <- venn.diagram(
	x = list(
		A.cervicornis = acerv$Orthogroup,
		M.cavernosa = mcav$Orthogroup,
		O.faveolata = ofav$Orthogroup,
		P.acuta = pacuta$Orthogroup,
		P.lobata = plob$Orthogroup
		),
	filename = "Output/Figs/Venn/venn_Orthogroup.png",
	col = "black",
