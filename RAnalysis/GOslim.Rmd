...
length(unique(mcav_all.go.slim.CC$GO.IDs)) # 21 unique CC GO terms 
...
length(unique(mcav_all.go.slim.MF$GO.IDs)) # 39 unique CC GO terms 
...
length(unique(ofav_all.go.slim.CC$GO.IDs)) # 3 unique CC GO terms 
...
length(unique(ofav_all.go.slim.MF$GO.IDs)) # 9 unique MF GO terms 
...
length(unique(mcap_all.go.slim.CC$GO.IDs)) # 40 unique CC GO terms 
...
length(unique(mcap_all.go.slim.MF$GO.IDs)) # 65 unique MF GO terms 
...
upset_go.bp <- upset(
  ...
  base_annotations=list(
    'Intersection size'=(     # this creates the bar plot showing intersection size (top right corner)
        ...
        bar_number_threshold=0.85,  # show all numbers on top of bars
        width=0.9,   # reduce width of the bars
        text = list(size=15), # change text size over bars
        text_colors = c(on_background = "black", on_bar = "white"), # change text color for text over bars
      )
      + scale_y_continuous(expand=expansion(mult=c(0, 0.05))) # add some space on the top of the bars
      + theme(
  ...
  themes = upset_modify_themes(
       list(
           'intersections_matrix' = theme(text = element_text(size = 55, face = "italic", colour = "black")), # text to the left of the matrix
          'overall_sizes' = theme(axis.text.x = element_text(angle = 0)) # text underneath set size plot
       )),
  sort_sets = 'descending', # sort sets in descending order
  sort_intersections = 'descending' # sort intersections in descending order
...
upset_go.bp
ggsave("../Output/Figs/upset/upset_GO.BP_20230906.pdf", upset_go.bp, width = 40, height = 20, units = c("in"))
ggsave("../Output/Figs/upset/upset_GO.BP_20230906.png", upset_go.bp, width = 40, height = 20, units = c("in"))
...
count_morph_plot
ggsave("Output/Figs/GOslim/BP/MorphologyCounts_GOslim.BP_20221227.pdf", count_morph_plot, width = 49, height = 40, units = c("in"))
ggsave("Output/Figs/GOslim/BP/MorphologyCounts_GOslim.BP_20221227.png", count_morph_plot, width = 49, height = 40, units = c("in"))
```

## Make contingency tables for BP (info [here](https://www.datacamp.com/community/tutorials/contingency-tables-r))
```{r}
# By location
...
tabl2
  #                                  Branching Intermediate Massive
  # cell adhesion                           28           12       6
  # cell cycle and proliferation            43           30      10
  # cell organization and biogenesis       105           34      58
  # cell-cell signaling                     26            4       6
  # death                                   15            6       5
  # developmental processes                635          100     134
  # DNA metabolism                          17            2      22
  # other biological processes             369           60     101
  # other metabolic processes              152           62      54
  # protein metabolism                      99           17      46
  # RNA metabolism                          91           14      22
  # signal transduction                     96           12      40
  # stress response                        109           17      21
  # transport                               81           26      22

rowSums(tabl2)
       #             cell adhesion     cell cycle and proliferation cell organization and biogenesis              cell-cell signaling 
       #                        46                               83                              197                               36 
       #                     death          developmental processes                   DNA metabolism       other biological processes 
       #                        26                              869                               41                              530 
       # other metabolic processes               protein metabolism                   RNA metabolism              signal transduction 
       #                       268                              162                              127                              148 
...
prop.table(table(all.go.BP.slim$GO.Slim.Term, all.go.BP.slim$Morphology)) * 100
  #                                    Branching Intermediate     Massive
  # cell adhesion                     0.99679601   0.42719829  0.21359915
  # cell cycle and proliferation      1.53079388   1.06799573  0.35599858
  # cell organization and biogenesis  3.73798505   1.21039516  2.06479174
  # cell-cell signaling               0.92559630   0.14239943  0.21359915
  # death                             0.53399786   0.21359915  0.17799929
  # developmental processes          22.60590958   3.55998576  4.77038092
  # DNA metabolism                    0.60519758   0.07119972  0.78319687
  # other biological processes       13.13634745   2.13599146  3.59558562
  # other metabolic processes         5.41117836   2.20719117  1.92239231
  # protein metabolism                3.52438590   0.60519758  1.63759345
  # RNA metabolism                    3.23958704   0.49839801  0.78319687
  # signal transduction               3.41758633   0.42719829  1.42399430
  # stress response                   3.88038448   0.60519758  0.74759701
  # transport                         2.88358847   0.92559630  0.78319687
...
# X-squared = 156.4, df = 26, p-value < 2.2e-16
```
...
## Make contingency tables for CC (info [here](https://www.datacamp.com/community/tutorials/contingency-tables-r))
```{r}
# By location
tabl1 <- table(all.go.CC.slim$GO.Slim.Term, all.go.CC.slim$Location)
tabl1
  #                              Florida Hawai'i
  # cytoskeleton                       3       6
  # cytosol                            0       6
  # ER/Golgi                          16      14
  # mitochondrion                      4      59
  # non-structural extracellular       6     105
  # nucleus                           28      28
  # other cellular component          51      77
  # other cytoplasmic organelle       13       9
  # other membranes                   25      82
  # plasma membrane                   15      40
  # translational apparatus            0       7

rowSums(tabl1)
     #            cytoskeleton                      cytosol                     ER/Golgi                mitochondrion non-structural extracellular 
     #                       9                            6                           30                           63                          111 
     #                 nucleus     other cellular component  other cytoplasmic organelle              other membranes              plasma membrane 
     #                      56                          128                           22                          107                           55 
...
prop.table(table(all.go.CC.slim$GO.Slim.Term, all.go.CC.slim$Location)) * 100
  #                                 Florida    Hawai'i
  # cytoskeleton                  0.5050505  1.0101010
  # cytosol                       0.0000000  1.0101010
  # ER/Golgi                      2.6936027  2.3569024
  # mitochondrion                 0.6734007  9.9326599
  # non-structural extracellular  1.0101010 17.6767677
  # nucleus                       4.7138047  4.7138047
  # other cellular component      8.5858586 12.9629630
  # other cytoplasmic organelle   2.1885522  1.5151515
  # other membranes               4.2087542 13.8047138
  # plasma membrane               2.5252525  6.7340067
  # translational apparatus       0.0000000  1.1784512

chisq.test(all.go.CC.slim$GO.Slim.Term, all.go.CC.slim$Location)
# Warning: Chi-squared approximation may be incorrect
# 	Pearson's Chi-squared test
# 
# data:  all.go.CC.slim$GO.Slim.Term and all.go.CC.slim$Location
# X-squared = 93.166, df = 10, p-value = 1.259e-15

# By morphology 
tabl2 <- table(all.go.CC.slim$GO.Slim.Term, all.go.CC.slim$Morphology)
tabl2
  #                              Branching Intermediate Massive
  # cytoskeleton                         3            4       2
  # cytosol                              0            6       0
  # ER/Golgi                            16           13       1
  # mitochondrion                       53            8       2
  # non-structural extracellular        92            0      19
  # nucleus                             28            3      25
  # other cellular component            54           36      38
  # other cytoplasmic organelle          9            9       4
  # other membranes                     58           36      13
  # plasma membrane                     33           10      12
  # translational apparatus              0            7       0
...
prop.table(table(all.go.CC.slim$GO.Slim.Term, all.go.CC.slim$Morphology)) * 100
  #                               Branching Intermediate    Massive
  # cytoskeleton                  0.5050505    0.6734007  0.3367003
  # cytosol                       0.0000000    1.0101010  0.0000000
  # ER/Golgi                      2.6936027    2.1885522  0.1683502
  # mitochondrion                 8.9225589    1.3468013  0.3367003
  # non-structural extracellular 15.4882155    0.0000000  3.1986532
  # nucleus                       4.7138047    0.5050505  4.2087542
  # other cellular component      9.0909091    6.0606061  6.3973064
  # other cytoplasmic organelle   1.5151515    1.5151515  0.6734007
  # other membranes               9.7643098    6.0606061  2.1885522
  # plasma membrane               5.5555556    1.6835017  2.0202020
  # translational apparatus       0.0000000    1.1784512  0.0000000

chisq.test(all.go.CC.slim$GO.Slim.Term, all.go.CC.slim$Morphology)
# Warning: Chi-squared approximation may be incorrect
# 	Pearson's Chi-squared test
# 
# data:  all.go.CC.slim$GO.Slim.Term and all.go.CC.slim$Morphology
# X-squared = 168.77, df = 20, p-value < 2.2e-16
```
...
count_morph_plot
ggsave("Output/Figs/GOslim/MF/MorphologyCounts_GOslim.MF_20221222.pdf", count_morph_plot, width = 49, height = 40, units = c("in"))
ggsave("Output/Figs/GOslim/MF/MorphologyCounts_GOslim.MF_20221222.png", count_morph_plot, width = 49, height = 40, units = c("in"))
```

## Make contingency tables for MF (info [here](https://www.datacamp.com/community/tutorials/contingency-tables-r))
```{r}
# By location
tabl1 <- table(all.go.MF.slim$GO.Slim.Term, all.go.MF.slim$Location)
tabl1
  #                                   Florida Hawai'i
  # cytoskeletal activity                   1       5
  # enzyme regulator activity              16       2
  # kinase activity                        10      54
  # nucleic acid binding activity          17      12
  # other molecular function              123     336
  # signal transduction activity           18     103
  # transcription regulatory activity       5       4
  # transporter activity                   10      22

rowSums(tabl1)
         #    cytoskeletal activity         enzyme regulator activity                   kinase activity     nucleic acid binding activity 
         #                        6                                18                                64                                29 
         # other molecular function      signal transduction activity transcription regulatory activity              transporter activity 
         #                      459                               121                                 9                                32 
...
prop.table(table(all.go.MF.slim$GO.Slim.Term, all.go.MF.slim$Location)) * 100
  #                                      Florida    Hawai'i
  # cytoskeletal activity              0.1355014  0.6775068
  # enzyme regulator activity          2.1680217  0.2710027
  # kinase activity                    1.3550136  7.3170732
  # nucleic acid binding activity      2.3035230  1.6260163
  # other molecular function          16.6666667 45.5284553
  # signal transduction activity       2.4390244 13.9566396
  # transcription regulatory activity  0.6775068  0.5420054
  # transporter activity               1.3550136  2.9810298

chisq.test(all.go.MF.slim$GO.Slim.Term, all.go.MF.slim$Location)
# Warning: Chi-squared approximation may be incorrect
# 	Pearson's Chi-squared test
# 
# data:  all.go.MF.slim$GO.Slim.Term and all.go.MF.slim$Location
# X-squared = 67.107, df = 7, p-value = 5.666e-12

# By morphology 
tabl2 <- table(all.go.MF.slim$GO.Slim.Term, all.go.MF.slim$Morphology)
tabl2
  #                                   Branching Massive
  # cytoskeletal activity                     5       1
  # enzyme regulator activity                 6      12
  # kinase activity                          24      40
  # nucleic acid binding activity            24       5
  # other molecular function                354     105
  # signal transduction activity             99      22
  # transcription regulatory activity         5       4
  # transporter activity                     13      19
...
prop.table(table(all.go.MF.slim$GO.Slim.Term, all.go.MF.slim$Morphology)) * 100
  #                                   Branching    Massive
  # cytoskeletal activity              0.6775068  0.1355014
  # enzyme regulator activity          0.8130081  1.6260163
  # kinase activity                    3.2520325  5.4200542
  # nucleic acid binding activity      3.2520325  0.6775068
  # other molecular function          47.9674797 14.2276423
  # signal transduction activity      13.4146341  2.9810298
  # transcription regulatory activity  0.6775068  0.5420054
  # transporter activity               1.7615176  2.5745257

chisq.test(all.go.MF.slim$GO.Slim.Term, all.go.MF.slim$Morphology)
# Warning: Chi-squared approximation may be incorrect
# 	Pearson's Chi-squared test
# 
# data:  all.go.MF.slim$GO.Slim.Term and all.go.MF.slim$Morphology
# X-squared = 81.44, df = 7, p-value = 7.003e-15
```
