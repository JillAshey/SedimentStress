...
#### Code for Francois sedimentation data. Pacuta only samples analyzed here aligned against Pacuta. STAR was read aligner.
...
```{r}
countdata <- read.csv("Output/DESeq2/pacuta/gene_count_pacuta_matrix.csv")
dim(countdata) # 35708 x 13
...
## Load metadata 
```{r}
metadata_pacuta <- read.csv("Data/pdam_metadata_raw_filtered_cleaned.csv")
metadata_pacuta <- na.omit(metadata_pacuta) # remove samples that have no metadata 
metadata_pacuta$SampleID <- gsub("X", "", metadata_pacuta$SampleID)
rownames(metadata_pacuta) <- metadata_pacuta$SampleID
metadata_pacuta$Species <- "Pocillopora acuta" # change species from pdam to pacuta 
```
...
write.csv(pacuta_counts_filt, "Output/DESeq2/pacuta/pacuta_counts_filt_20220113.csv")
...
DEG_control_vs_mid.sig.num # 80 DEGs
...
write.csv(DEG_control_vs_mid.sig.list_full, "Output/DESeq2/pacuta/pacuta_C_vs_Mid_DEG_20220113.csv")
```
...
DEG_control_vs_high.sig.num # 226 DEGs
...
write.csv(DEG_control_vs_high.sig.list_full, "Output/DESeq2/pacuta/pacuta_C_vs_High_DEG_20220113.csv")
```
...
DEG_mid_vs_high.sig.num # 6 DEGs
...
write.csv(DEG_mid_vs_high.sig.list_full, "Output/DESeq2/pacuta/pacuta_Mid_vs_High_DEG_20220113.csv")
```
...
dim(DEGs.all) # 312 x 20
length(unique(DEGs.all$gene_id)) # 232 unique genes between all treatments 
write.csv(DEGs.all, "Output/DESeq2/pacuta/pacuta_DEGs_AllTreatments_20220113.csv")

# Bind all genes
gene.all <- rbind(DEG_control_vs_mid,
                  DEG_control_vs_high,
                  DEG_mid_vs_high)
write.csv(gene.all, "Output/DESeq2/pacuta/pacuta_AllGenes_AllTreatments_20220113.csv")
```
...
dim(DEGs.all_pacuta) # 232 unique DEGs among treatment comparisons 
```

## 
```{r}
unique.sig.list <- data[which(rownames(data) %in% DEGs.all_pacuta$DEGs), ] # subset list of sig transcripts from original count data
write.csv(counts(unique.sig.list), file = "Output/DESeq2/pacuta/pacuta_unique.sig.list_20220113.csv")
...
ggsave("Output/Figs/pacuta/pacuta_DEGs_PCA_20220113.jpeg", pacuta_DEGPCAplot, width = 25, height = 25, units = "cm")
ggsave("Output/Figs/pacuta/pacuta_DEGs_PCA_20220113.pdf", pacuta_DEGPCAplot, width = 25, height = 25, units = "cm")
```
