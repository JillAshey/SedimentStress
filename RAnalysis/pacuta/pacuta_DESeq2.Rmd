...
```{r}
## Load data 
DEG <- read.csv("Output/DESeq2/pacuta/pacuta_DEGs_AllTreatments_20220116.csv", header = T)
head(DEG)
...
## Plot basic scatter plot: x is log2FoldChange, y is pvalue (I'm using padj in my plots)
ggplot(data=DEG, aes(x=log2FoldChange, y=padj)) + geom_point()

## Convert the p-value into a -log10(p-value)
p <- ggplot(data=DEG, aes(x=log2FoldChange, y=-log10(padj))) + geom_point() + theme_minimal()
p
