...
# Code for Francois sedimentation data. P. dam only samples analyzed here aligned against P. dam. HISAT2 was read aligner with gff annotation file from NCBI.
...
countdata <- read.csv("Desktop/PutnamLab/Repositories/Tufts_URI_RNAseq/Tufts_URI_CSM_RNASeq/Seneca/STAR_pipeline/Output/DESeq2/hisat2/gene_count_pdam_NCBI_hisat2_matrix.csv", header = TRUE, row.names = "gene_id")
dim(countdata) # 23079 x 64
...
dim(gkeep) # 15996 genes left after filtering
...
write.csv(gcount_filt, "~/Desktop/gene_count_pdam_NCBI_hisat2_filtered.csv")
...
ggsave("~/Desktop/pdam_heatmap_NCBI_hisat2_PCA.pdf", pdam_heatmap_PCA, width = 8, height = 8, units = c("in"))
