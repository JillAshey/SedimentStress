...
```{r}
ofav_ID <- metadata_ofav$SampleID
count_ofav <- select(countdata, all_of(ofav_ID))
all(rownames(metadata_ofav) %in% colnames(count_ofav)) # must come out TRUE
```
...
write.csv(ofav_counts_filt, "Output/DESeq2/ofav/ofav_counts_filt_20211104.csv")
# Checking to make sure rownames in metadata == colnames in counts data 
all(rownames(metadata_ofav) %in% colnames(ofav_counts_filt)) # must come out TRUE
```

## Set Treatment as a factor
```{r}
metadata_ofav$Treatment <- factor(metadata_ofav$Treatment, levels = c("control", "Treatment1", "Treatment2", "Treatment3", "Treatment4"))
data <- DESeqDataSetFromMatrix(countData = ofav_counts_filt, colData = metadata_ofav, design = ~ Treatment)
```
...
DEG_control_vs_T1.sig.num # 8 DEGs
...
write.csv(DEG_control_vs_T1.sig.list_full, "Output/DESeq2/ofav/ofav_C_vs_T1_DEG_20211104.csv")
```
...
DEG_control_vs_T2.sig.num # 7 DEGs
...
write.csv(DEG_control_vs_T2.sig.list_full, "Output/DESeq2/ofav/ofav_C_vs_T2_DEG_20211104.csv")
```
...
DEG_control_vs_T3.sig.num # 12 DEGs
...
write.csv(DEG_control_vs_T3.sig.list_full, "Output/DESeq2/ofav/ofav_C_vs_T3_DEG_20211104.csv")
```
...
DEG_control_vs_T4.sig.num # 10 DEGs
...
write.csv(DEG_control_vs_T4.sig.list_full, "Output/DESeq2/ofav/ofav_C_vs_T4_DEG_20211104.csv")
```
...
length(unique(int1)) # 0 genes shared between control_vs_T1 and control_vs_T2'
...
int4 <- intersect(DEG_control_vs_T2.sig.list_full$gene_id, DEG_control_vs_T3.sig.list_full$gene_id)
length(unique(int4)) # 6 genes shared between control_vs_T2 and control_vs_T3
int5 <- intersect(DEG_control_vs_T2.sig.list_full$gene_id, DEG_control_vs_T4.sig.list_full$gene_id)
length(unique(int5)) # 5 genes shared between control_vs_T2 and control_vs_T4
int6 <- intersect(DEG_control_vs_T3.sig.list_full$gene_id, DEG_control_vs_T4.sig.list_full$gene_id)
length(unique(int6)) # 9 genes shared between control_vs_T3 and control_vs_T4
...
dim(DEGs.all) # 37 x 23
length(unique(DEGs.all$gene_id)) # 16 unique genes between all treatments 
write.csv(DEGs.all, "Output/DESeq2/ofav/ofav_DEGs_AllTreatments_20211104.csv")
...
gene.all <- rbind(DEG_control_vs_T1,
                  ...
                  DEG_control_vs_T3,
                  DEG_control_vs_T4)
write.csv(gene.all, "Output/DESeq2/ofav/ofav_AllGenes_AllTreatments_20211104.csv")
```
...
dim(DEGs.all_ofav) # 16 unique DEGs among treatment comparisons 
```
...
ggsave("Output/Figs/ofav/ofav_DEGs_PCA_20211104.pdf", ofav_DEGPCAplot, width = 10, height = 10, units = "cm")
```
...
Both volcano plots (DEG and all genes) look the same...need to look into this more
