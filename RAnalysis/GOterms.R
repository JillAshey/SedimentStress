...
acerv_IPS_GO$GO.ID <- gsub(",", ";", acerv_IPS_GO$GO.ID)
acerv_IPS_GO <- unique(acerv_IPS_GO)
dim(acerv_IPS_GO) # 20494 x 2
...
dim(acerv_BU_GO) # 2172 x 2
length(unique(acerv_BU_GO$gene_id)) # 2172

# Old GO terms 
acerv_oldGO <- read.csv("~/Desktop/PutnamLab/Repositories/SedimentStress/SedimentStress/Output/GOSeq/acerv/acerv_old_GOterms.csv", header = T)
acerv_oldGO <- select(acerv_oldGO, c("prot", "GO_term"))
colnames(acerv_oldGO) <- c("gene_id", "GO.ID")
acerv_oldGO$GO.ID <- gsub(",", ";", acerv_oldGO$GO.ID)
acerv_oldGO <- unique(acerv_oldGO)
dim(acerv_oldGO) # 20494 x 2
length(unique(acerv_oldGO$gene_id)) # 12828
# I think that acerv_IPS_GO and acerv_oldGO are the same. acerv_oldGO must have been generated by IPS by someone else 

# Bind IPS, BU and old GO terms by row
acerv_GO <- rbind(acerv_BU_GO, acerv_IPS_GO, acerv_oldGO)
acerv_GO <- unique(acerv_GO)
#acerv_GO$GO.ID <- gsub("NA", "", acerv_GO$GO.ID)
acerv_GO <- separate_rows(acerv_GO, GO.ID, sep = ";")
acerv_GO <- unique(acerv_GO)
acerv_GO <- aggregate(acerv_GO$GO.ID, list(acerv_GO$gene_id), paste, collapse = ";")
colnames(acerv_GO) <- c("gene_id", "GO.ID")

# Save as a csv
write.csv(acerv_GO, file = "~/Desktop/acerv_GO_20210219.csv")
